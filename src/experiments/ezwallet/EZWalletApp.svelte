<script lang="ts">
  ...
  function makeOne() {
    ...
    link = `${location.origin}/ezwallet/drop?p=${p}`;
...
<div class="ezwallet-shell">
  ...
  <header class="glass-panel hero-panel">
    ...
    <p class="hero-tag">Scan. Receive. Send. Stellar.</p>
    <p class="hero-sub">Public event QR drops for XLM and USDC.</p>
  </header>

  <section class="dashboard-grid">
    <article class="glass-panel">
      <h2 class="section-title">01 · QR Generator</h2>
      ...
      <label>Mode
        <select bind:value={mode}>
          <option value="receive_request">Receive Request</option>
    ...
    <article class="glass-panel">
      <h2 class="section-title">02 · QR Preview</h2>
    ...
    <article class="glass-panel">
      <h2 class="section-title">03 · Bulk Event Mode</h2>
      <label>Plan
    ...
    <article class="glass-panel pricing-panel">
      <h2 class="section-title">04 · Pricing & Payment</h2>
      <div class="plan-grid">
      ...
      <p class="muted">Platform payments remain separate from QR intents. No external processors.</p>
    </article>
  </section>
  ...
  <footer class="glass-panel trust-strip">
    ...
    <span class="trust-chip">Intent Links, Not Auto-Payments</span>
  </footer>
</div>

<style>
  .ezwallet-shell { color: #f4f4f5; max-width: 1200px; margin: 0 auto; padding: 1.25rem; position: relative; }
  .stellar-orbit-bg { position: absolute; inset: 0; pointer-events: none; background: radial-gradient(circle at 10% 0%, #1d2330aa, transparent 35%), radial-gradient(circle at 90% 20%, #7dd3fc22, transparent 28%), #030305; z-index: -1; }
  ...
  input, select { width: 100%; margin-top: .35rem; background: #0b0d12; color: #f4f4f5; border: 1px solid rgba(255,255,255,.18); border-radius: 10px; padding: .58rem .66rem; }
  ...
  .holo-button { background: linear-gradient(110deg, #f8fafc, #9ca3af, #ffffff, #b6c7ff, #fef3c7, #cbd5e1); color:#060709; font-weight:700; }
