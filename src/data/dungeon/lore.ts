...
export const dungeonLore: Record<DungeonRoomId, RoomLore> = {
  ...
  intake: {
    ...
    verifierExplainer:
      "Verifier: Groth16 (Circom / BN254). A fresh proof is generated locally on every attempt. With a passkey wallet, it goes on-chain: the Tier Verifier contract runs the full Groth16 pairing equation on Stellar Mainnet using CAP-0074 native BN254 host functions — built for this credential scheme specifically. The core capability is balance threshold proofs without revealing the balance. That pattern is broadly useful: private stake thresholds in DAO votes, collateral tier verification in lending protocols, tiered content access where the qualifying amount should stay off the record. The verification key is stable and the proof is compact — this is the kind of gate you deploy and leave alone.",
    failureExplainersByReasonCode: {
      PROOF_INVALID:
  ...
  catalog: {
    ...
    verifierExplainer:
      “Verifier: UltraHonk (Noir / bb.js). Runs locally in-browser for high-throughput policy checks, and settles on-chain via a Noir verifier contract — Soroban's first live UltraHonk prover, deployed because nothing equivalent existed on Stellar. The key difference from Room 1: this enforces exact membership, not minimum threshold. That distinction matters more than it sounds. Regulatory bucketing requires proving you are in bracket X, not just ≥ X. Role-gated DeFi pools may be non-compliant if you are over-qualified. Jurisdiction attestations often require a specific tier where a higher one is not a substitute. UltraHonk handles those cases; the Noir circuit lives off-chain and can be updated; the deployed verifier stays put.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
  ...
  cold: {
    ...
    verifierExplainer:
      “Verifier: RISC0 zkVM (WASM). Checks receipts locally in-browser. On-chain, the architecture is distinct from both rooms above: RISC0 compresses its STARK receipt into a Groth16 proof with five standard public inputs (control root hi/lo, claim digest hi/lo, BN254 control ID), and a Soroban contract runs the full pairing check on Stellar Mainnet. That contract routes through farm-attestations — a verification-key registry that tracks proof systems by ID (R0G16V1 for RISC0, NOIR_ROLE_V1 for Noir, with room for more). New circuits can be onboarded by registering a key; the attestation layer never redeploys. One registry, many proof systems. That is the headline. Prove an off-chain ML result. Record a game outcome. Attest a credit score or a sensor reading. Anything a RISC0 program can compute, farm-attestations can permanently record on Stellar. For this demo, pre-generated samples keep play fast; when the prover service is live, fresh receipts are generated per attempt and submitted through these same contracts.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
