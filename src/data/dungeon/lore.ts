...
export const dungeonLore: Record<DungeonRoomId, RoomLore> = {
  ...
  cold: {
    ...
    verifierExplainer:
      “Verifier: RISC0 zkVM (WASM). Two separate proof paths run in parallel. Local door verification uses a STARK receipt — RISC0's native output, verifiable in-browser by the WASM verifier but not suitable for on-chain submission. On-chain settlement uses a separate pre-wrapped Groth16 proof (pi_a, pi_b, pi_c, five BN254 public inputs) that the Soroban contract can verify directly via pairing check — this fires whenever wallet is connected and the ADVANCED ON-CHAIN toggle is enabled, with no VPS required. The prover service, when running, replaces that bundled Groth16 proof with a fresh one generated from your actual wallet inputs; without it, the bundled proof goes on-chain instead. Non-blocking. The contract routes through farm-attestations, a verification-key registry keyed by system ID (R0G16V1 for RISC0, NOIR_ROLE_V1 for Noir). New proof systems onboard by registering a key; the attestation layer never redeploys. Prove an off-chain ML result, a game outcome, a credit score, a sensor reading — anything a RISC0 program can compute, farm-attestations can record on Stellar.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
