  ...
  cold: {
    ...
    briefingMarkdown:
      [
        ...
        “They chose parity — EVEN or ODD based on your tier ID. It is not sophisticated. That is the point. The RISC0 zkVM handles it because zkVM receipts can attest to arbitrary computation, which means the policy program can be upgraded without touching the verifier contract. What makes this room remarkable: a custom Soroban contract implements the full Groth16 pairing verification equation from scratch, purpose-built for RISC0's standard five-input encoding — control root, claim digest, and execution identity, all bound into the proof. The contract runs a complete BN254 pairing check on Stellar Mainnet using CAP-0074 host functions, the same primitive as Room 1 but a distinct, separately deployed contract. Arbitrary computation, cryptographically settled on Stellar.”,
    ...
    protocolPlacard:
      ...
      “Verifier: RISC0 zkVM (WASM). Local verification checks the receipt directly in-browser using the RISC0 WASM verifier. On-chain verification is a different beast: RISC0 compresses its STARK receipt into a Groth16 proof with five standard public inputs — control root (hi/lo), claim digest (hi/lo), and BN254 control ID, all cryptographically binding the execution trace and program identity. A custom-built Soroban contract, written from scratch, runs the full Groth16 pairing equation on those inputs on Stellar Mainnet using CAP-0074 native BN254 pairing. This is not a hash record — it is a complete pairing check. The contract routes through farm-attestations, a purpose-built VK registry contract that manages verification keys by ID (R0G16V1 for RISC0), allowing multiple proof systems to share a single attestation layer. For this demo, the dungeon uses pre-generated sample proofs to keep play fast without a live prover service. When the prover VPS is running, the dungeon generates fresh RISC0 receipts per attempt, wraps them as Groth16, and submits for full on-chain verification — same contracts, same pairing check, live wallet inputs.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
