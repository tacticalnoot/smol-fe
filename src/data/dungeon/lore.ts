...
export const dungeonLore: Record<DungeonRoomId, RoomLore> = {
  ...
  cold: {
    ...
    verifierExplainer:
      “Verifier: RISC0 zkVM (WASM). Two proof paths run in parallel. Local door verification uses a STARK receipt — RISC0's native output, checked in-browser by the WASM verifier. On-chain settlement uses a separately bundled Groth16 proof (pi_a, pi_b, pi_c, five BN254 public inputs) verified on Stellar Mainnet via pairing check — fires whenever wallet and ADVANCED ON-CHAIN toggle are on, no VPS required. To be direct about scope: the contracts are built, the Groth16 prover host is written and tested, the VPS architecture is wired, and the on-chain submission path works end-to-end. What is not connected for this submission is the dungeon's per-door proving loop calling the live prover instead of the bundled sample — that integration would make each door attempt generate a fresh, wallet-bound receipt rather than reusing the pre-generated one. It is a real next step, not a missing piece of the system. The hard parts are done. The contract routes through farm-attestations, a verification-key registry keyed by system ID (R0G16V1 for RISC0, NOIR_ROLE_V1 for Noir) — new proof systems onboard by registering a key, no redeploy. Prove an off-chain ML result, a game outcome, a credit score, a sensor reading — anything a RISC0 program can compute, farm-attestations can record on Stellar.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
