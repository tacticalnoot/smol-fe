...
export const dungeonLore: Record<DungeonRoomId, RoomLore> = {
  ...
  cold: {
    ...
    verifierExplainer:
      “Verifier: RISC0 zkVM (WASM). Two proof paths run in parallel. Local door verification uses a STARK receipt — RISC0's native output, checked in-browser by the WASM verifier. On-chain settlement uses a separately bundled Groth16 proof (pi_a, pi_b, pi_c, five BN254 public inputs) verified on Stellar Mainnet via pairing check — fires whenever wallet and ADVANCED ON-CHAIN toggle are on, no VPS required. To be direct about scope: Room 1 already demonstrates the full live-proving pattern — a fresh, wallet-bound Groth16 proof generated in-browser on every door attempt, verified on-chain before you can advance. For Room 3, the contracts are built, the Groth16 prover host is written and tested, and the on-chain submission path works end-to-end with the bundled proof. What is not connected is the dungeon's per-door loop calling the live RISC0 prover instead of the pre-generated receipt — the same integration Room 1 already does, applied to the zkVM side. That is the remaining wire. The hard parts are done. The contract routes through farm-attestations, a verification-key registry keyed by system ID (R0G16V1 for RISC0, NOIR_ROLE_V1 for Noir) — new proof systems onboard by registering a key, no redeploy. Prove an off-chain ML result, a game outcome, a credit score, a sensor reading — anything a RISC0 program can compute, farm-attestations can record on Stellar.”,
    failureExplainersByReasonCode: {
      PROOF_INVALID:
