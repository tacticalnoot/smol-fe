    ...
    const leaderboardData: Readable<LeaderboardEntry[]> = derived(
        ...
        ([$smols, $users]) => {
            ...
            for (const smol of $smols) {
                if (smol.Address && typeof smol.Plays === 'number') {
                    ...
                    console.warn("Smol object missing Address or has invalid Plays count:", smol);
                }
            }
    ...
    <div class="max-w-[1024px] mx-auto">
        ...
        {:else}
            <div class="overflow-x-auto shadow-lg rounded-lg">
                <table class="min-w-full bg-slate-800 border border-slate-700">
                    <thead class="bg-slate-700">
                        <tr>
                            ...
                            <th class="text-right py-3 px-4 md:px-6 font-semibold uppercase tracking-wider text-sm">Songs Created</th>
            ...
            <p class="text-xs text-slate-500 mt-4 text-center">
                Leaderboard reflects stats based on the provided song and user data.
            </p>
        {/if}
    </div>
</div>
