<script lang="ts">
    ...
    async function handleSignUpSubmit() {
        ...
        try {
            ...
            const now = new Date();
            const cutoff = new Date("2026-02-15"); // Feb 14th cutoff
            if (now < cutoff) {
                // "Assign to account" (Simulated via Preferences/Local for now)
                // In a real backend, this would be: await api.grantTrait(user.id, 'valentine_2026');
...
<div class="w-full min-h-screen">
    {#if address}
        <!-- Render ArtistResults as if viewing the user's own artist page -->
        <div class="pt-4 pb-24">
            <ArtistResults {address} />
        </div>
    {:else}
        <!-- Pixel Theme Glassmorphism Splash -->
        <div
                                ...
                                <div class="py-1">
                                    {#each bgOptions as bg, i}
                                        ...
                                        >
                                            ...
                                            {#if bg.special}
                                                <!-- Final Fantasy Style Notification Badge -->
                                                <div
