<script lang="ts">
  ...
  async function handlePublish() {
    ...
    if (mixtapeDraftState.tracks.length === 0) {
    ...
    try {
      const published = await publishMixtape(getSnapshotDraft());
      clearDraft();
  ...
  function handleExternalDrop(event: DragEvent) {
    ...
    try {
      ...
      if (snapshot.tracks.length === 0 || !tracksListEl) {
        mixtapeDraftState.tracks = [...mixtapeDraftState.tracks, normalized];
        tracksForDnd = getSnapshotDraft().tracks;
        return;
      }
