    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" on:click={handleClose}>
        <div class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl" on:click|stopPropagation>
            ...
            <div class="sticky top-0 z-10 flex items-center justify-between border-b border-slate-700 bg-slate-900 p-6">
                <div>
                    <h2 class="text-2xl font-bold text-white">Purchase Mixtape</h2>
            ...
            <div class="p-6 space-y-6">
                {#if tracksToMint.length === 0 && tracksToPurchase.length === 0}
                    <div class="text-center py-8">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-400/20 mb-4">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-8 h-8 text-emerald-400">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                            </svg>
                        </div>
                        <p class="text-lg font-semibold text-white">All Set!</p>
                        <p class="text-sm text-slate-400 mt-2">You already own all tracks in this mixtape.</p>
                    </div>
                {:else}
                    <!-- Summary Card -->
                    <div class="rounded-xl border border-slate-700 bg-slate-800/50 p-4">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p class="text-xl font-bold text-white">{tracksToMint.length + tracksToPurchase.length}</p>
                            </div>
                            <div>
                                <p class="text-slate-400">Total cost</p>
                                <p class="text-xl font-bold text-lime-400">{totalCost} KALE</p>
                            </div>
                            <div>
                                <p class="text-slate-400">Signatures required</p>
                                <p class="text-lg font-semibold text-white">{totalSignatures}</p>
                            </div>
                            <div>
                                    ...
                                    <div class="flex items-start gap-3">
                                        <div class="flex-shrink-0 mt-0.5">
                                            {#if isCompleted}
                                                <div class="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-400">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 text-slate-900">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            {#if step.substeps && (isCurrent || isCompleted) && isProcessing}
                                                <div class="mt-3 space-y-1.5 pl-2 border-l-2 {isCurrent ? 'border-lime-400/30' : 'border-emerald-400/30'}">
                                                    {#each step.substeps as substep}
                                                        {@const subCompleted = completedSteps.has(substep.id)}
                                                        <div class="flex items-center gap-2 text-sm">
                                                            {#if subCompleted}
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 text-emerald-400 flex-shrink-0">
                                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                                                </svg>
                                                                <span class="text-emerald-400">{substep.label}</span>
                                                            {:else}
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 text-slate-600 flex-shrink-0">
                                                                    <circle cx="10" cy="10" r="3" />
                                                                </svg>
                                                                <span class="text-slate-400">{substep.label}</span>
                                                            {/if}
                                                        </div>
                                                    {/each}
                                                </div>
                    ...
                    {#if isProcessing}
                        <div class="rounded-lg border border-blue-500/50 bg-blue-500/10 p-4">
                            <div class="flex items-start gap-3">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5">
            ...
            <!-- Footer -->
            <div class="sticky bottom-0 z-10 flex gap-3 border-t border-slate-700 bg-slate-900 p-6">
                {#if tracksToMint.length > 0 || tracksToPurchase.length > 0}
                    {#if !isProcessing}
                        <button
                    ...
                    {:else}
                        <div class="flex-1 rounded-lg border border-slate-600 bg-slate-800 px-4 py-2.5 text-sm font-semibold text-slate-400 text-center">
