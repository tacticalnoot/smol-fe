    ...
    async function startGameLocal(opts?: { fromRemote?: boolean }) {
        ...
        if (!runId || !runStartedAt) {
            ...
            runStartedAt = Date.now();
            {
                ...
                noirRoleSalt = String(new DataView(bytes.buffer).getUint32(0, false));
        ...
        if (accepted) {
            ...
            if (currentFloor === 2 && isConnected && advancedOnChainGuards) {
                ...
                try {
                    ...
                    let proofOverride: { proofBase64: string; publicInputs: string[] };
                    ...
                    try {
                        ...
                        const proverJson = await proverResp.json().catch(() => null);
                        if (!proverResp.ok || !proverJson?.ok) {
                            throw new Error(proverJson?.error || "Prover unavailable");
                        }

                        proofOverride = {
                            proofBase64: String(proverJson.sample?.proof?.proofBase64 ?? ""),
                            publicInputs: Array.isArray(proverJson.sample?.proof?.publicInputs) ? proverJson.sample.proof.publicInputs : [],
                        };
                        if (!proofOverride.proofBase64 || proofOverride.publicInputs.length === 0) {
                            throw new Error("Prover returned an invalid proof payload");
                        }
                    } catch {
                        // Contest-friendly fallback: use a bundled training proof (real verifier still runs).
                        usedTrainingNoir = true;
                        const sample =
                            noirUltraHonkRoleLegacyBundle.samples.find((s) => s.expectedValid) ?? noirUltraHonkRoleLegacyBundle.samples[0];
                        proofOverride = {
                            proofBase64: String(sample?.proof?.proofBase64 ?? ""),
                            publicInputs: Array.isArray(sample?.proof?.publicInputs) ? sample.proof.publicInputs : [],
                        };
                        if (!proofOverride.proofBase64 || proofOverride.publicInputs.length === 0) {
                            throw new Error("No bundled Noir training proof available");
                        }
                    }
                    ...
                    addLogEntry({
                        ...
                        attempt: attempts,
                        result: "correct",
                        proofType: usedTrainingNoir ? "On-chain UltraHonk (Noir) [training]" : "On-chain UltraHonk (Noir)",
                        txHash: submitRes.txHash,
                        timestamp: Date.now(),
                        verified: true,
                        provingTimeMs: result.provingTimeMs,
            ...
            if (currentFloor === 3 && isConnected && advancedOnChainGuards) {
                ...
                try {
                    ...
                    try {
                        ...
                        receiptJson = String(proverJson.proof?.receipt_json ?? "");
                        if (!receiptJson) throw new Error("Prover did not return receipt_json");

                        const { risc0MethodIdHex: methodId } = await import("../../../../data/the-farm/risc0Bundle");
                        risc0MethodIdHex = methodId;

                        proofOverride = {
                            claim_digest_hex: String(proverJson.proof?.claim_digest_hex ?? ""),
                            public_inputs_hex: Array.isArray(proverJson.proof?.public_inputs_hex) ? proverJson.proof.public_inputs_hex : [],
                            proof: {
                                pi_a_b64: String(proverJson.proof?.proof?.pi_a_b64 ?? ""),
                                pi_b_b64: String(proverJson.proof?.proof?.pi_b_b64 ?? ""),
                                pi_c_b64: String(proverJson.proof?.proof?.pi_c_b64 ?? ""),
                            },
                        };
                    } catch {
                        // Contest-friendly fallback: verify a bundled training receipt locally.
                        usedTrainingRisc0 = true;
                        const { risc0MethodIdHex: methodId, risc0Samples } = await import("../../../../data/the-farm/risc0Bundle");
                        risc0MethodIdHex = methodId;
                        const sample = (risc0Samples as any[]).find((s) => s?.expectedValid) ?? (risc0Samples as any[])[0];
                        receiptJson = String(sample?.proof?.receiptJson ?? "");
                        if (!receiptJson) throw new Error("No bundled RISC0 training receipt available");
                        proofOverride = undefined;
                    }

                    // Integrity: locally verify the receipt before submitting on-chain.
                    const { verifyRisc0Receipt } = await import("../../../../lib/the-farm/verifiers/risc0");
                    const localVerify = await verifyRisc0Receipt(risc0MethodIdHex, { receiptJson });
                    if (!localVerify.valid) {
                    ...
                    addLogEntry({
                        ...
                        attempt: attempts,
                        result: "correct",
                        proofType: usedTrainingRisc0 ? "On-chain RISC0 Groth16 Receipt [training]" : "On-chain RISC0 Groth16 Receipt",
                        txHash: submitRes.txHash,
                        timestamp: Date.now(),
                        verified: true,
                        provingTimeMs: result.provingTimeMs,
        ...
        <div class="dg-how-it-works">
            ...
            <div class="dg-hiw-item">
                ...
                <div>
                    ...
                    <p>Room 1 uses Groth16 per attempt. Rooms 2–3 can generate live-input Noir/RISC0 proofs if the local prover is running, then submit real mainnet on-chain verification via passkey.</p>
                </div>
            </div>
            <div class="dg-hiw-item">
