<script lang="ts">
    ...
    async function chooseDoor(doorIndex: number) {
        ...
        if (accepted) {
            ...
            //
            // Note: the proof is generated by a DEV-only local prover service (Node + WSL) and
            // is bound to this run's inputs (door policy + tierId + salt).
            if (currentFloor === 2 && isConnected) {
                ...
                try {
                    ...
                    if (!proverResp.ok || !proverJson?.ok) {
                        throw new Error(
                            proverJson?.error ||
                                "Local Noir prover unavailable. In DEV, ensure WSL is installed and try again.",
                        );
                    }
            ...
            //
            // Note: the receipt+Groth16 proof is generated by a DEV-only local prover service (Node + WSL)
            // and is bound to this run's inputs (tier/balance/threshold/salt).
            if (currentFloor === 3 && isConnected) {
                ...
                try {
                    ...
                    if (!proverResp.ok || !proverJson?.ok) {
                        throw new Error(
                            proverJson?.error ||
                                "Local RISC0 prover unavailable. In DEV, ensure WSL + Rust toolchain is installed and try again.",
                        );
                    }

                    // Integrity: locally verify the returned receipt before submitting on-chain.
                    const receiptJson = String(proverJson.proof?.receipt_json ?? "");
                    if (!receiptJson) throw new Error("Local RISC0 prover did not return receipt_json");
                    const { risc0MethodIdHex } = await import("../../../../data/the-farm/risc0Bundle");
