...
// Existing mainnet ultrahonk-verifier contract ID (written by deploy script).
...
// IMPORTANT: must match on-chain `admin()` for the chosen contract.
...
async function main() {
  ...
  if (!contractId) {
    throw new Error(
      "Missing ultrahonk-verifier contract ID. Set ULTRAHONK_VERIFIER_CONTRACT_ID_MAINNET or ensure deployed-ultrahonk-verifier-id.txt exists.",
    );
  }

  console.log("Upgrade: ultrahonk-verifier (mainnet)");
  ...
  // 3) Upload wasm (idempotent by hash)
