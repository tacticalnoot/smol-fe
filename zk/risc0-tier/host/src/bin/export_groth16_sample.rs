...
fn main() -> Result<()> {
    ...
    // Deterministic sample (matches the tier bundles conceptually).
    // (tier_index, threshold, balance, salt_bytes32)
    let tier_index: u8 = 1;
    let threshold: u64 = 100;
    let balance: u64 = 220;
    let salt: [u8; 32] = [22u8; 32];
    ...
    let out = OutputJson {
        ...
        claim_digest_hex: bytes_to_hex(claim_digest.as_bytes()),
        public_inputs_hex,
        proof: ProofJsonOut {
